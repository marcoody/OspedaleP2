...
Turno *UserBuilder::buildT(const QString & tipoTurno, Turno *prec){
    ...
    if(tl && (tipoTurno!="Libero"))
        return new Turno_straordinario(tl->getGiornoTurno());

    if(ts && tipoTurno == "Straordinario")
        return new Turno_straordinario(ts->getGiornoTurno(), ts->getNOre(), ts->getReparto(), ts->getPermesso(), ts->getVolontariato(), ts->getPagaStraordinario());


    if(tl && tipoTurno == "Libero")
        return new Turno_libero(tl->getGiornoTurno(), tl->getPermesso());


    if(ti && tipoTurno =="Intero")
        return new Turno_intero(ti->getGiornoTurno(), ti->getNOre(), ti->getReparto(), ti->getPagaIntero());

    if(tp && tipoTurno == "Parziale")
        return new Turno_parziale(tp->getGiornoTurno(), tp->getNOre(),tp->getReparto(), tp->getPagaParziale());

    if(tipoTurno == "Intero")
        return new Turno_intero(prec->getGiornoTurno(), prec->getNOre());

    if(tipoTurno == "Parziale")
        return new Turno_parziale(prec->getGiornoTurno(), prec->getNOre());
